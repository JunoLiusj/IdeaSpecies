...
``samples``  one row per independent generation
             ...
             optional: condition columns, valuable column (0/1 gold label of the idea;
...
``counts``   one row per observed idea
             ...
             optional: condition columns, valuable column (0/1 gold label per idea)
...
def load_conditions(
    ...
    for key, g in groups:
        ...
        if fmt == "samples":
            out.append(_from_samples(g, label, keys, idea_col, valuable_col, str(path)))
        else:
            out.append(_from_counts(g, label, keys, idea_col, count_col, valuable_col, str(path)))
